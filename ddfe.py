...
SERPAPI_KEY = "a85863a47a0d1d66c9b25d483a16900623de6d572b14d1bd76608aa8b7fca5fc"
GOOGLE_API_KEY = "AIzaSyAV3iPqeB0eyvs93x3teT-AYNxIcJMVUEQ"
...
st.sidebar.header("📇 Save Your Plan / Contact Us")
...
VENDASTA_WEBHOOK_URL = "https://automations.businessapp.io/start/LCZN/75206877-c089-4a69-a881-9df69c75ff71"
...
def send_to_vendasta(first_name, last_name, email, phone):
    ...
    try:
        response = requests.post(VENDASTA_WEBHOOK_URL, json=payload)
        if response.status_code == 200:
            st.sidebar.success("✅ Contact info sent to our CRM!")
        else:
            st.sidebar.error(f"❌ Failed to send. Status: {response.status_code}")
    except Exception as e:
        st.sidebar.error(f"🚨 Error: {e}")

if st.sidebar.button("📬 Send My Info"):
    if all([first_name, last_name, email, phone]):
        send_to_vendasta(first_name, last_name, email, phone)
    else:
        st.sidebar.warning("⚠️ Please complete all fields.")